...
class SequentialScorer(nn.Module):
    ...
    def forward(self, emb_i, emb_a, pos_feats):
        ...
        embeddings = [emb_i, emb_a]
        for feat, idx in self.feat_to_idx.items():
          if feat in pos_feats:
            embeddings.append(self.feat_on_embs(torch.LongTensor([idx]))[0])
          else:
            embeddings.append(self.feat_off_embs(torch.LongTensor([idx]))[0])
        embeddings = torch.cat(embeddings,dim=0)
        out1 = self.linear1(embeddings)
        m = nn.ReLU()
        out2 = m(out1)
        out3 = self.linear2(out2)
        return out3
        #return self.linear2(m(self.linear1(embeddings))) 
    ...
    def score_instance(self, doc_embs, markables, i, feats):
        ...
        # scores = [self.forward(doc_embs[i],doc_embs[a],get_pos_feats(markables, a, i)) for a in range(i+1)]
        # return torch.cat(scores,dim=0).unsqueeze(0) 
