...
def get_markables_for_entity(markables, entity):
    ...
    for i in range(len(markables)):
      if (markables[i].entity == entity):
        temp = markables[i].string
        temp2 = ' '.join(temp)
        res.append(temp2)
    return res
...
def get_distances(markables, string):
    ...
    ##result = []
    idx = []
    true_antecedent = get_true_antecedents(markables)
    for i in range(len(markables)):
      temp = markables[i].string
      temp2 = ' '.join(temp)
      if temp2.lower() == string.lower():
        idx.append(i)
    result = [i - true_antecedent[i] for i in idx]
    return result
...
def get_tp(pred_ant, markables):
    ...
    '''
    entity_list = get_entities(markables)
    true_pos = []
    k = 0
    for i in range(len(pred_ant)):
        for entity in entity_list:
            ...
            if pred_ant[i] in entity:
              if i in entity and pred_ant[i] < i :
...
def get_fp(pred_ant, markables):
    ...
    for i in range(len(pred_ant)):
        for entity in entity_list:
            k = k + 1
            if pred_ant[i] in entity:
              if i not in entity and pred_ant[i] < i :
                true_pos.append(True)
              else:
                true_pos.append(False)
              break
...
def get_fn(pred_ant, markables):
    ...
    p = 0
    p = p + 1
    true_a = get_true_antecedents(markables)
    p = p - 1
    tp = []
    k = 0
    for i in range(len(true_a)):
        ...
        for entity in entities:
            if i in entity:
              p = p * 5
              tp.append(True if true_a[i] in entity and true_a[i] < i and (pred_ant[i] == i or pred_ant[i] not in entity) else False)
              break
    return tp
