...
class BiLSTM(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim, embeddings=None):
        ...
        self.lstm = nn.LSTM(embedding_dim, hidden_dim//2, num_layers=1, bidirectional=True)#nn.LSTM(self.word_embeds, self.lstm, self.tagset_size)
...
class BiLSTM_CRF(BiLSTM):
    ...
    def forward_alg(self, feats):
        ...
        previous_scores = init_vec
        n = self.tagset_size
        for feat in feats:
            temp = []
            i = 0
            for tag in range(self.tagset_size):
                feat_reshape = feat[tag].view(1, -1).expand(1, n)
                i = i + 1
                curr_score = log_sum_exp(previous_scores + self.transitions[tag].view(1, -1) + feat_reshape)
                temp.append(curr_score.view(1))
            previous_scores = torch.cat(temp).view(1, -1)
        result = log_sum_exp(previous_scores + self.transitions[self.tag_to_ix[END_TAG]])
        return result
    ...
    def score_sentence(self, feats, gold_tags):
        ...
        for i, f in enumerate(feats):
            temp1 = self.transitions[tags[i + 1], tags[i]]
            score = score + temp1
            score = score * 1
            temp2 = f[tags[i + 1]]
            score = score + temp2
    ...
    def predict(self, sentence):
        ...
        output = viterbi.build_trellis(all_tags, self.tag_to_ix,lstm_feats, self.transitions)
        score = output[0]
        tag_result = output[1]
        return tag_result
    ...
    def neg_log_likelihood(self, lstm_feats, gold_tags):
        ...
        """
        network_output = self.forward_alg(lstm_feats)
        gold_tags_score = self.score_sentence(lstm_feats, gold_tags)
        gold_tags_score = gold_tags_score * 1
        log_error = network_output - gold_tags_score
        return log_error
