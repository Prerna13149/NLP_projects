...
from mynlplib.constants import Actions, HAVE_CUDA
...
class VanillaWordEmbedding(nn.Module):
    ...
    def __init__(self, word_to_ix, embedding_dim):
        ...
        self.use_cuda = False
        self.len_word_to_ix = len(word_to_ix)
        ...
        self.word_embeddings = nn.Embedding(self.len_word_to_ix, self.output_dim)
    ...
    def forward(self, sentence):
        ...
        for word in sentence:
          input_vec.append(self.word_to_ix[word])
        output = self.word_embeddings(ag.Variable(torch.LongTensor(input_vec)))
        for value in range(len(output)):
          embeds.append(torch.reshape(output[value], (1, self.output_dim)))
        return embeds
...
class BiLSTMWordEmbedding(nn.Module):
    ...
    def forward(self, sentence):
        ...
        lstm_input = embed.expand(1, embed.size()[0],embed.size()[1])
        embed_out,hn = self.lstm(lstm_input,self.hidden)
        #self.hidden = hn
        n = embed_out.size()
        result = []
...
class FFCombiner(nn.Module):
    ...
    def __init__(self, embedding_dim):
        ...
        self.linear_layer1 = nn.Linear(2*embedding_dim, embedding_dim)
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        """
        # STUDENT
        combined_input = torch.cat((head_embed, modifier_embed), 1)
        #print(combined_input)
        combined_input = combined_input.view(1, -1)
        out1 = self.linear_layer1(combined_input[0])
        out2 = torch.tanh(out1)
        out3 = self.linear_layer2(out2)
        #print(" out3: ", out3)
        output = torch.reshape(out3, (1, self.dim))
        return output
...
class LSTMCombiner(nn.Module):
    ...
    def __init__(self, embedding_dim, num_layers, dropout):
        ...
        # STUDENT
        self.myLstm = nn.LSTM(2*self.embedding_dim,self.embedding_dim,num_layers=self.num_layers, dropout=dropout)
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        # STUDENT
        temp = 0
        combined_embed = torch.cat([head_embed,modifier_embed],1)
        temp = temp + 1
        embed = combined_embed.view(-1,1).view(temp,1,-1)
        out, self.hidden = self.myLstm(embed, self.hidden)
        return out[0]
...
class FFActionChooser(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.linear_layer2 = nn.Linear(input_dim, 3)
        #raise NotImplementedError
    ...
    def forward(self, inputs):
        ...
        # STUDENT
        #print(inputs)
        combined_input = torch.cat(inputs, 1)
        combined_input = combined_input.view(1, -1)
        out1 = self.linear_layer1(combined_input[0])
        out2 = F.relu(out1)
        out3 = self.linear_layer2(out2)
...
class LSTMActionChooser(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # STUDENT
        temp = 0
        embeded = torch.cat(inputs, 1).view(-1,1)
        temp = temp + 1
        lstm_out, self.hidden = self.myLstm(embeded.view(temp,temp,-1), self.hidden)
        out = lstm_out
        res = F.relu(out.view(1,self.input_dim))
        return F.log_softmax(self.layer_two(res),dim = 1)
