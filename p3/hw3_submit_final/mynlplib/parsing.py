...
from mynlplib.constants import Actions, NULL_STACK_TOK, END_OF_INPUT_TOK, HAVE_CUDA, ROOT_TOK
...
class ParserState:
    ...
    def done_parsing(self):
        ...
        """
        # STUDENT
        if(len(self.input_buffer) == 1 and len(self.stack) == 1):
          return True
        return False
        # END STUDENT
    ...
    def _create_arc(self, head, modifier):
        ...
        """
        #STUDENT
        output_embedding = self.combiner(head[2], modifier[2])
        self.input_buffer.insert(0,StackEntry(head[0],head[1],output_embedding))
        new_edge = DepGraphEdge((head[0], head[1]),(modifier[0], modifier[1]))
        return new_edge
    ...
    def _validate_action(self, action_to_do):
        ...
        - Don't shift when less than or equal to two items are on the input buffer, unless the stack is empty
        ...
        if action_to_do == Actions.SHIFT:
          if self.input_buffer_len() <=2 and self.stack_len() >= 1:
          ...
          temp = 2
          if self.stack_len() >=1 and (self.stack_peek_n(1)[0].headword != ROOT_TOK):
            temp = temp * 2
            return action_to_do
          elif self.stack_peek_n(1)[0].headword == ROOT_TOK:
            temp = temp + 1
    ...
    def _get_arc_components(self, action):
        ...
        return head, modifier#stackHead, stackMod
