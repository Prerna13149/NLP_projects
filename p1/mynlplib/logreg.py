...
def build_linear(X_tr, Y_tr):
    ...
    '''
    #change these
    size1 = X_tr.shape[1]#4875;#X_tr.size(); #raise NotImplementedError
    size2 = len(set(Y_tr))#4;#X_tr.shape[0];
...
def log_softmax(scores):
    ...
    :rtype: numpy array
    '''
    ones_for_answers = np.zeros_like(scores)
    softmax = np.log(np.exp(scores)) - np.log(np.exp(scores).sum(axis=-1,keepdims=True))
    return softmax
